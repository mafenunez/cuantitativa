...
proc_elsoc <- elsoc_2021 %>%
    select(
        idencuesta,
        ing_per = m13, ing_per_just = m15,
        edad = m0_edad, mesfuerzo = c18_09, mtalento = c18_10
    ) %>%
    mutate(
...
cor(x = proc_elsoc$ing_per, y = proc_elsoc$ing_per_just, use = "complete.obs")
```

```{r, include=FALSE, warning=FALSE}
r_coef <- cor(x = proc_elsoc$ing_per, y = proc_elsoc$ing_per_just, use = "complete.obs")
...
sjPlot::plot_scatter(data = proc_elsoc, x = ing_per, y = ing_per_just)
```
...
proc_elsoc2 <- proc_elsoc %>% filter(!ing_per_just %in% maximo)

sjPlot::plot_scatter(data = proc_elsoc2, x = ing_per, y = ing_per_just)

cor(x = proc_elsoc2$ing_per, y = proc_elsoc2$ing_per_just, use = "complete.obs")
```
...
cor_results <- cor.test(proc_elsoc$ing_per, proc_elsoc$ing_per_just,
    method = "pearson",
...
Para ejemplificar, tomaremos una de las limitaciones más conocidas: **el coeficiente de correlación de Pearson es sensible a valores extremos**.
...
sjPlot::plot_scatter(data = proc_elsoc, x = ing_per, y = ing_per_just) +
    geom_text(aes(label = "← Ingresos = 750.000 e Ingresos justos = 50.000.000"),
        x = 5500000, y = 50100000, size = 3, hjust = 1, color = "black"
    )
```
...
max_ing_per_just <- proc_elsoc %>% filter(ing_per_just == max(ing_per_just, na.rm = T))

# Excluye este caso de proc_elsoc
proc_elsoc2 <- proc_elsoc %>% filter(!idencuesta %in% max_ing_per_just$idencuesta)
...
```{r, include=FALSE}
r_coef2 <- cor(x = proc_elsoc2$ing_per, y = proc_elsoc2$ing_per_just, use = "complete.obs")
...
Una de las limitaciones más conocidas del coeficiente de correlación es que un mismo valor puede representar distintas distribuciones de datos. El mejor ejemplo de esta limitación es el cuarteto de Anscombe, que muestra cuatro conjuntos de datos con las mismas propiedades estadísticas (media, mediana y varianza), pero con distribuciones muy diferentes.
