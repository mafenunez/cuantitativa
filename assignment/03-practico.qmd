...
load(url("https://dataverse.harvard.edu/api/access/datafile/6160180")) # Cargar bbdd pública de ELSOC

# Procesemos la bbdd quedandonos solo con algunas variables de interés
proc_elsoc <- elsoc_2021 %>%
    select(
        idencuesta, region, comuna,
        estrato:fact_exp02,
        sexo = m0_sexo, edad = m0_edad, educ = m01,
        hrs_trabajo = m12, hrs_cuidado = m17,
        ing_per = m13, ing_per_justo = m15, ing_hog = m29,
        mesfuerzo = c18_09, mtalento = c18_10, ess = d01_01,
        satis_demo = c01, frec_marcha = c08_02
    ) %>%
    mutate(
        across(sexo:ess, ~ if_else(. %in% c(-666, -777, -888, -999), NA, .)), # Recodificar a NA
...
```{r include = F}
rows <- NROW(proc_elsoc)
cols <- NCOL(proc_elsoc)
```
...
cor(x = proc_elsoc$hrs_trabajo, y = proc_elsoc$ing_per, use = "complete.obs")
```

Tenemos que la correlación entre la variable de `hrs_trabajo` (horas semanales dedicadas al trabajo en la ocupación) y `ing_per` (ingresos mensuales del entrevistado) es de 0.26. 
...
sjPlot::plot_scatter(data = proc_elsoc, x = hrs_trabajo, y = ing_per)
```
...
Tenemos que la correlación entre la variable de horas semanales dedicadas al trabajo en la ocupación y los ingresos mensuales es de 0.26. ¿Cómo interpreto esto?

Una manera recomendable es la siguiente:

_El coeficiente de correlación de Pearson entre las horas semanales dedicadas al trabajo en la ocupación y los ingresos mensuales es positivo y pequeño (r = 0.26) según Cohen (1988)._ 
...
Tomemos por ejemplo la siguiente pregunta de investigación: _¿en qué medida el nivel educacional alcanzado por las personas se relaciona con su estatus social subjetivo en Chile en el 2021?_
...
Obtengamos el coeficiente de correlación $r$ de Pearson entre el nivel educacional alcanzado (`educ`) y el estatus social subjetivo (`ess`) de las personas en Chile en 2021. Para esto usaremos solamente observaciones completas (_listwise_).

```{r, echo=TRUE}

cor_results <- cor.test(proc_elsoc$educ, proc_elsoc$ess,
    method = "pearson",
    use = "complete.obs"
)
...
> El coeficiente de correlación de Pearson entre nivel educativo y el estatus social subjetivo es positivo, estadísticamente significativo (r = 0.21, p < .001) y pequeño de acuerdo con las recomendaciones de Cohen (1988). Por tanto, con un 95% de confianza se puede rechazar la $H_{0}$ de no asociación entre variables, existiendo evidencia a favor de la $H_{A}$ sobre una asociación significativa entre nivel educacional y estatus social subjetivo.
...
En nuestro ejemplo anterior entre estatus social subjetivo y años de educación, teníamos que su coeficiente de correlación era $r = 0.21$
...
Con esto, podemos decir que el 4.5% de la variabilidad del estatus social subjetivo es compartido con la variabilidad en el nivel educativo.
...
Cuando queremos conocer la asociación entre variables que son ordinales y/o cuando nuestras variables no cumplen con los supuestos de distribución normal, podemos utilizar la **correlación de Spearman** $\rho$.
...
cor.test(proc_elsoc$mesfuerzo, proc_elsoc$mtalento,
    use = "complete.obs",
    method = "spearman"
) # especificamos metodo spearman
```

Ahora conocemos el valor del coeficiente mediante al argumento `rho`, que es igual a 0.61, siendo positivo y grande según los criterios de Cohen (1988). 
...
A partir de la base de datos `proc_elsoc` responda la siguiente pregunta _¿en qué medida la satisfacción con la democracia de las personas está asociada a la frecuencia con que participan en marchas pacificas?_ Para responder esta pregunta calcule el coeficiente de correlación de Spearman ($\rho$) entre las variables `satis_demo` y `frec_marcha`. Luego siga los mismos pasos señalados en el Ejercicio 1.
